...
# for Ableton 11
# 5 scenes, no mute and solo buttons
...
##########
    ...
    def _create_controls(self):
        ...
        #self._sustain_button = make_button(0,97, resource_type=SharedResource, name='Sustain_Button') #non funziona
        ...
        #self._lesolobuttons=[ make_stop_button(0, 0 + index, name='Track_Solo_%d' % (index + 1)) for index in range(self.SESSION_WIDTH)]
        #self._lemutebuttons=[ make_stop_button(0, 8 + index, name='Track_Mute_%d' % (index + 1)) for index in range(self.SESSION_WIDTH)]
        ...
        if self.HAS_TRANSPORT:
            #self._sustain_button = make_color_button(0, 88, name = 'Sustain_Button')
            self._play_button = make_on_off_button(0, 91, name='Play_Button')
            self._session_record_button = make_on_off_button(0, 93, name='Session_Record_Button')
            self._record_button = self.make_shifted_button(self._session_record_button)    
        ...
        self._scene_launch_buttons = [make_color_button(0, 82 + index, name='Scene_Launch_%d' % (index + 1)) for index in range(5)] #self.SESSION_HEIGHT)]
        ...
        #self._select_button.add_value_listener(self._undo_func, identify_sender = True)
        ...
        self._stop_all_button.add_value_listener(self._press_stop_all, identify_sender = False) #serafino
        self._metronome_button = self.make_shifted_button(self._play_button)

        #self._shift_button.add_value_listener(self._press_stop, identify_sender = False)
    ...
    def _undo_func(self,value, *a, **b):
        #if  self._shift_button.is_pressed() and self._stop_all_button.is_pressed(): # and self._leSelectBut._last_received_value==0
        if  self._shift_button.is_pressed(): #and self._leSelectBut._last_received_value==0:
    ...
    def _press_stop_all(self,value):
        self.make_del_but(value)
        #self._undo_func(self, value)
        return

    # def make_copyPaste_butt(self,value):
    #     if self._shift_button.is_pressed():
    #         #self._session.set_clip_stop_buttons(self._null_button)
    #         #self._session.set_clip_stop_buttons(self._stop_all_button)
    #         self.show_message('MADONNA TROIA')
    ...
    #                 slot.set_copyPaste_button(self._stop_all_button)
    ...
    def _create_transport(self):
        ...
        def play_toggle_model_transform(value):
            if self._shift_button.is_pressed():
                return False
            return value
        transport=TransportComponent(name='Transport', is_enabled=False, play_toggle_model_transform=play_toggle_model_transform, layer=Layer(play_button=self._play_button, session_record_button=self._session_record_button, record_button=self._record_button, metronome_button=self._metronome_button))
        return transport
