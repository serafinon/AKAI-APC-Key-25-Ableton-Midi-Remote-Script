    ...
    def _launch_button_value(self, value):
        ...
        if self.is_enabled():
            ...
            if self._select_button and self._select_button.is_pressed() and value:
                ...
                #self.show_message('DUPLICATE')

                #doppio click con shift
                if (time.time()-self.startTime_shift)<.4:
                    #self.show_message('DUPLICATE')
                    self.log_message("duplicate")
                    self._do_duplicate_clip()
                else:
                    self.startTime_shift = time.time()
                ...
                if self._duplicate_button and self._duplicate_button.is_pressed():
                    ...
                    #     self.delnow=True;
                    #     self.delTime=time.time()
                    if self._feedback_value()!=self._started_value:
                        self.log_message("Start")
                        if self._feedback_value()==self._recording_value:
                           self.log_message("Recording in progress")
                           self.stop_rec()
